...
#
# =============================================================================
# THIS FILE'S PURPOSE (Day 6 - Test 1 of 2): PROVE THE EXPLICIT "FAIL" PATH
# =============================================================================
# What we're doing here: deliberately typing a WRONG password instead of the
# real one, so the actual live site shows its own genuine
# "Incorrect username/password" error banner - no redirect happens, we stay
# on the login page.
#
# What we're checking: does the agent correctly notice that error message
# and declare "action": "fail" on its own - a real, reasoned conclusion that
# the goal failed - instead of either (a) blindly retrying the same failed
# submission forever, or (b) just silently running out of MAX_TURNS with an
# "inconclusive" verdict that doesn't actually mean anything was diagnosed.
#
# This is the negative-path mirror of Day 3's "done" recognition test - same
# idea, but proving the agent can recognize FAILURE from real evidence too,
# not just success.
# =============================================================================
#
...
# This file only ever runs the WRONG PASSWORD test - that toggle is ON by
# default here. The bad-selector toggle stays OFF in this file on purpose;
# that's a separate experiment, tested in its own separate file
# (day6_ui_agent_ReAct_selfcorrect.py), so each file proves exactly one thing.
TEST_FORCE_WRONG_PASSWORD = True     # ON in this file - this IS the experiment
TEST_FORCE_BAD_SELECTOR_ONCE = False  # OFF here - not what this file is testing
