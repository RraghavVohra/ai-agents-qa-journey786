...
#
# =============================================================================
# THIS FILE'S PURPOSE (Day 6 - Test 2 of 2): PROVE SELF-CORRECTION FROM MEMORY
# =============================================================================
# What we're doing here: forcing exactly ONE deliberately-wrong selector
# (#this-id-does-not-exist) the first time the agent tries to click the
# "terms and conditions checkbox" - a fake ID guaranteed not to exist on the
# real page. Playwright will genuinely time out trying to find it. This is
# NOT the same as Test 1 - there, the SITE produced a real failure (wrong
# credentials). Here, WE artificially inject a failure into an otherwise
# correct action, to test the agent's own reaction to it.
#
# What we're checking: because of the Day 6 fix that made execution errors
# RECOVERABLE (the run no longer instantly dies on a failed action), the
# agent gets another turn. The real question - the whole point of building
# ReAct - is whether the NEXT turn's reasoning actually references this
# failure and either explains it or lands on a different/correct selector,
# versus just blindly repeating the identical broken selector out of luck.
# That difference is the actual proof memory is doing something, not just
# decoration.
# =============================================================================
#
...
# --- TEST HARNESS toggles ----------------------------------------------------
# This file only ever runs the BAD SELECTOR test - that toggle is ON by
# default here. The wrong-password toggle stays OFF; that's Test 1's
# separate experiment, already proven in day6_ui_agent_ReAct_wrongpassword.py.
# Each file proves exactly one thing, on purpose.
TEST_FORCE_WRONG_PASSWORD = False     # OFF here - not what this file is testing
TEST_FORCE_BAD_SELECTOR_ONCE = True   # ON in this file - this IS the experiment
